...
Ansible code shared with the public. Includes playbooks for installing deviceConnect on macOS hosts and gigacap on Ubuntu hosts.
...
├── inventory.yml               # Host inventory (you must edit this)
...
The inventory file `inventory.yml` defines two host groups: `mac` (macOS hosts for deviceConnect) and `gem` (Ubuntu hosts for gigacap). Edit it to match your environment.

1. Open `inventory.yml` in your editor. The file has the following structure:
...
2. Update the hosts under each group to match your environment. For each host, set `ansible_host` to its IP address or hostname. Add or remove host entries as needed.

3. If needed, update the `ansible_user` under each group's `vars` section to match the SSH user for that group.

4. The SSH passwords are referenced as variables (`mac_password` and `gem_password`). Supply them at runtime using one of:
...
Both playbooks are safe to re-run. If a run fails mid-way, the installer file may be left in `/tmp` on the target host. A subsequent successful run will clean it up.
